...
private:
    ByteVector read_block_impl(std::uintptr_t address, std::size_t length) override {
        ...
        return {begin(data_) + address, begin(data_) + address + length};
    }
...
TEST_CASE("Dereference pointers") {
    ...
    mio->write(1000, i_ptr_ptr);
    mio->write(reinterpret_cast<std::uintptr_t>(i_ptr_ptr), i_ptr);
    mio->write(reinterpret_cast<std::uintptr_t>(i_ptr), i);
