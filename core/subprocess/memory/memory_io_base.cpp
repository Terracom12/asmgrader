...
ByteVector MemoryIOBase::read_until(std::uintptr_t address, const std::function<bool(std::byte)>& predicate) {
    ByteVector result;

    bool pred_satisfied = false;
    for (std::uintptr_t current_address = address; !pred_satisfied; current_address += 8) {
        auto next_block = read_block_impl(current_address, 8);
...
ByteVector MemoryIOBase::read_until(std::uintptr_t address,
    ...
    for (std::uintptr_t current_address = address;; current_address += block_size) {
        ByteVector block = read_block_impl(current_address, block_size);
