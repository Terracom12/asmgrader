...
public:
    ...
    std::vector<T> read_array(std::uintptr_t address, std::size_t size) {
        std::vector<T> result;
        result.reserve(size);

        for (std::uintptr_t current_address = address; size > 0; current_address += sizeof(T), --size) {
            result.push_back(MemoryIOSerde<T>::read(current_address, *this));
        }
    ...
    template <MemoryReadSupportedType T>
        requires std::is_trivial_v<T> // more complex types may have sizes that don't correspond
    std::vector<T> read_array(std::uintptr_t address, std::function<bool(const T&)> until_predicate) {
        std::vector<T> result;

        for (std::uintptr_t current_address = address;; current_address += sizeof(T)) {
            T elem = MemoryIOSerde<T>::read(current_address, *this);
    ...
    ByteVector read_bytes(std::uintptr_t address, std::size_t length) { return this->read_block_impl(address, length); }

    // FIXME: The reinterpret_cast's to/from std::uintptr_t and pointer types seems kinda bad
    template <MemoryReadSupportedType T>
    util::remove_all_pointers_t<T> read_deref_all(std::uintptr_t address) {
        if constexpr (std::is_pointer_v<T>) {
            using NextType = std::remove_pointer_t<T>;

            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
            auto next_address = reinterpret_cast<std::uintptr_t>(read<T>(address));

            return read_deref_all<NextType>(next_address);
        } else {
    ...
    std::size_t write(std::uintptr_t address, const T& data) {
        ByteVector bytes = MemoryIOSerde<T>::to_bytes(data);
        this->write_block_impl(address, bytes);
        return bytes.size();
    }
...
private:
    ...
    virtual ByteVector read_until(std::uintptr_t address, const std::function<bool(std::byte)>& predicate);
    virtual ByteVector read_until(std::uintptr_t address,
                                  const std::function<bool(std::span<const std::byte>)>& predicate,
                                  std::size_t block_size);

    virtual ByteVector read_block_impl(std::uintptr_t address, std::size_t length) = 0;
    virtual void write_block_impl(std::uintptr_t address, const ByteVector& data) = 0;
};
