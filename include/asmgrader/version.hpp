...
constexpr std::string_view format_as(const CompilerInfo::Vendor& from) {
...
consteval buildinfo::BuildInfo get_build_info() {
...
consteval auto get_dependencies() {
    return detail::convert_deps_str<detail::static_deps_str>();
}

consteval AppMode get_app_mode() {
    return get_build_info().app_mode;
}

consteval bool is_prof_mode() {
    return get_app_mode() == AppMode::Professor;
}

consteval bool is_student_mode() {
    return get_app_mode() == AppMode::Student;
}

consteval auto get_plain_version_str() {
...
inline std::string get_version_str() {
    constexpr auto build_info = get_build_info();
...
{
    static auto debug_format(const ::asmgrader::buildinfo::Dependency& from, format_context& ctx) {
        return fmt::format_to(ctx.out(), "Dependency{{.name = {:?}, .version = {:?}}}", from.name, from.version);
    }

    static auto normal_format(const ::asmgrader::buildinfo::Dependency& from, format_context& ctx) {
        return fmt::format_to(ctx.out(), "{} {}", from.name, from.version);
    }

    auto format(const ::asmgrader::buildinfo::Dependency& from, format_context& ctx) const {
        if (is_debug_format) {
...
{
    static auto debug_format(const ::asmgrader::buildinfo::CompilerInfo& from, format_context& ctx) {
        return fmt::format_to(ctx.out(), "CompilerInfo{{.vendor = {:?}, .major = {}, .minor = {}, .patch = {}}}",
                              from.vendor, from.major, from.minor, from.patch);
    }

    static auto normal_format(const ::asmgrader::buildinfo::CompilerInfo& from, format_context& ctx) {
        return fmt::format_to(ctx.out(), "{} v{}.{}.{}", from.vendor, from.major, from.minor, from.patch);
    }

    auto format(const ::asmgrader::buildinfo::CompilerInfo& from, format_context& ctx) const {
        if (is_debug_format) {
            return debug_format(from, ctx);
        }
        return normal_format(from, ctx);
...
{
    static auto debug_format(const ::asmgrader::buildinfo::BuildInfo& from, format_context& ctx) {
    ...
    static auto normal_format(const ::asmgrader::buildinfo::BuildInfo& from, format_context& ctx) {
        // Only include first 12 chars from git hash
        ctx.advance_to(fmt::format_to(ctx.out(), "AsmGrader v{}.{}.{}-g{:.12} ({})\n", from.major, from.minor,
                                      from.patch, from.git_hash, from.app_mode));
        ...
        *ctx.out()++ = '\n';
        ctx.advance_to(fmt::format_to(ctx.out(), "Built: {} {} ({})\n", asmgrader::buildinfo::get_date_iso8601(),
                                      from.time, from.date));

        return ctx.out();
    }

    auto format(const ::asmgrader::buildinfo::BuildInfo& from, format_context& ctx) const {
        if (is_debug_format) {
            return debug_format(from, ctx);
        }
        return normal_format(from, ctx);
