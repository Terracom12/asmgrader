...
struct TimeoutError
{
    ...
    [[maybe_unused]] friend std::string format_as(const TimeoutError& from) {
        return fmt::format("TimeoutError ({}): {}", from.duration, from.info);
    }
};
...
[[maybe_unused]] inline std::string format_as(const PtraceEvent& from) {
    ...
    case Seccomp:
        return "seccomp";
    default:
        return fmt::format("<unknown ({})>", fmt::underlying(from));
    }
}
...
struct TracedWaitid
{
    ...
    static util::linux::Result<TracedWaitid> waitid(idtype_t idtype, id_t id, int options = WSTOPPED | WEXITED) {
    ...
    template <util::ChronoDuration Duration1, util::ChronoDuration Duration2 = std::chrono::milliseconds>
    static util::Expected<TracedWaitid, TimeoutError>
    wait_with_timeout(pid_t pid, Duration1 timeout, Duration2 poll_period = std::chrono::milliseconds{5}) {
        ...
        while (high_resolution_clock::now() - start_time < timeout) {
            util::linux::Result<siginfo_t> waitid_res = util::linux::waitid(P_PID, pid, WEXITED | WSTOPPED | WNOHANG);

            util::assert_expected(waitid_res);
        ...
        return TimeoutError{.duration = std::chrono::duration_cast<std::chrono::milliseconds>(timeout),
                            .info = "waitid"};
    }
    ...
    constexpr static TracedWaitid parse(const siginfo_t& siginfo) {
        ...
        constexpr uint64_t SIG_MASK = 0x7f;
        constexpr uint64_t SYSCALL_TRAP_MASK = 0x80;
        uint32_t signal_bits = siginfo.si_status;
    ...
    [[maybe_unused]] friend std::string format_as(const TracedWaitid& from) {
        ...
        return fmt::format("TracedWaitid{{type={}, exit_code={}, signal={}, is_syscall_trap={}, ptrace_event={}}}",
                           type_to_str(from.type), from.exit_code, from.signal_num, from.is_syscall_trap,
                           from.ptrace_event);
    }
};
