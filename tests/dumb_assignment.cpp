...
TEST("sum function") {
    ...
    REQUIRE(sum(1, 1) == 2, "1 + 1 = 2");
    REQUIRE(sum(10000, 9999) == 19999, "10000 + 9999 = 19999");

    REQUIRE(sum_and_write(0xAB, 0xCD), "function runs!");
    REQUIRE(ctx.get_stdout() == "\xAB\0\0\0\0\0\0\0\0\xCD\0\0\0\0\0\0\0\0", "stdout's cool");
    ...
    REQUIRE(syscalls.size() == 2, "2 syscalls!");
}
